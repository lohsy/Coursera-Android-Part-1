...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		startActivityForResult(explicitIntent, ActivityLoaderActivity.GET_TEXT_REQUEST_CODE);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Log.i(TAG, "Entered onActivityResult()" + requestCode + " " + resultCode +
				" " + RESULT_OK + " " + ActivityLoaderActivity.GET_TEXT_REQUEST_CODE + " "
		+ data.getStringExtra(ActivityLoaderActivity.TAG));

		// TODO - Process the result only if this method received both a
		// RESULT_OK result code and a recognized request code
		// If so, update the Textview showing the user-entered text.
		if(resultCode == RESULT_OK && requestCode == ActivityLoaderActivity.GET_TEXT_REQUEST_CODE){
			Log.i(TAG, "Entered onActivityResult()");
			String text = data.getStringExtra(ActivityLoaderActivity.TAG);
